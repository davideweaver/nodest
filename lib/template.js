...
class ActiveTemplate {
	...
	*/
	partial(name) {
		return this.template.controller.template(name).generate(this._data);
	}
	...
	*/
	rawPartial(name) {
		return this.template.controller.rawTemplate(name).generate(this._data);
	}
	...
	*/
	child() {
			return this.template.controller.template(this.template._children).generate(this._data)
	}
...
class Template {
	...
	*/
	generate(data) {
		...
		let vars = this.controller ? this.controller.vars : null;
		...
		return new ActiveTemplate(this, vars, data).generate();
	}
